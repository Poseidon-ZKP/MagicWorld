...
export async function dnld_crypto_files(cardNum: number) {
  try {
    ...
    return {
      encrypt_wasm: new Uint8Array(encrypt_wasm),
      encrypt_zkey: new Uint8Array(encrypt_zkey),
      decrypt_wasm: new Uint8Array(decrypt_wasm),
      decrypt_zkey: new Uint8Array(decrypt_zkey),
    };
  } catch (e) {
