...
async function deployShuffleEncrypt() {
  const vk = await (
    await ethers.getContractFactory('ShuffleEncryptVerifierKey')
  ).deploy();
  return await (
    await ethers.getContractFactory('Shuffle_encryptVerifier', {
      libraries: {
...
async function deployShuffleEncryptCARD30() {
  return await (
    await ethers.getContractFactory('Shuffle_encryptVerifier_30card')
  ).deploy();
}
